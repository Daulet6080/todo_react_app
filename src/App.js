...
export default function App() {
  ...
  const [editOpened, setEditOpened] = useState(false); // State for edit modal
  ...
  const [taskSummary, setTaskSummary] = useState(""); // State for task summary
  const [taskDeadline, setTaskDeadline] = useState(""); // State for deadline
  const [taskState, setTaskState] = useState("Not done"); // Default state
  const [currentTaskIndex, setCurrentTaskIndex] = useState(null); // To track which task is being edited
  ...
  const editTask = (index) => {
    ...
    setTaskState(task.state); // Set state to the selected task's state
    setTaskDeadline(task.deadline);

    setCurrentTaskIndex(index);  // Set current task index for updating
    setEditOpened(true);  // Open the Edit task modal
  };
  ...
  const updateTask = () => {
    ...
    updatedTasks[currentTaskIndex] = updatedTask; // Update the task at the specific index
    setTasks(updatedTasks);
    saveTasks(updatedTasks);
    setEditOpened(false); // Close the Edit modal
  };
